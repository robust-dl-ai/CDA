...
def _work(process_id, infer_dataset, args):
    ...
    for iter, pack in enumerate(infer_data_loader):
        ...
        ###################修改代码#########################
        cam_dict = np.load(os.path.join(args.cam_out_aug_dir, img_name + '.npy'), allow_pickle=True).item()
        ###################修改代码#########################
        ...
        ###################修改代码#########################
        imageio.imwrite(os.path.join(args.ir_label_out_aug_dir, img_name + '.png'),
...
def run(args):
    dataset = dataloader.VOC12ImageDataset(args.train_list, voc12_root=args.voc12_root, img_normal=None,
                                           to_torch=False)
