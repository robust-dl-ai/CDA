...
def data_aug(origin_img, fg_img, fg_seg):
...
def data_aug_rotation_blur(origin_img, fg_img, fg_seg):
    bg_img = np.empty_like(origin_img)
    bg_img[:, :, :] = origin_img[:, :, :]
    ...
    ################blur#################
    fg_seg[fg_seg != 0] = 255
    fg_seg = cv2.GaussianBlur(fg_seg, (3, 3), 0, 0, cv2.BORDER_DEFAULT) / 255
    fg_seg = np.expand_dims(fg_seg, 2).repeat(3, axis=2)

    bg_img_modify = bg_img[row:row + resize_shape[0], col:col + resize_shape[1], :]
    bg_img[row:row + resize_shape[0], col:col + resize_shape[1], :] = fg_seg * fg_img + (1 - fg_seg) * bg_img_modify
